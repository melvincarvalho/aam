 ...
 * @param {string} repo - GitHub repo in format "owner/repo", full GitHub URL, custom URL, or local path
...
function parseRepo(repo) {
  ...
  // Check for local paths first (file://, ./, ../, or absolute paths)
  if (repo.startsWith('file://')) {
    isLocalPath = true;
    localPath = repo.slice(7); // Remove 'file://'
    skillName = path.basename(localPath);
    owner = 'local';
    repoName = skillName;
    fullRepo = `local/${skillName}`;
    return { owner, repoName, fullRepo, skillName, version, isCustomUrl, customUrl, isLocalPath, localPath };
  }
  ...
  if (repoWithoutVersion.includes('://')) {
    ...
    const githubMatch = repoWithoutVersion.match(/github\.com\/([^\/]+)\/([^\/\.]+)/);
    // Check for GitLab URL
    const gitlabMatch = repoWithoutVersion.match(/gitlab\.com\/([^\/]+)\/([^\/\.]+)/);
    // Check for Codeberg URL
    const codebergMatch = repoWithoutVersion.match(/codeberg\.org\/([^\/]+)\/([^\/\.]+)/);
    ...
    if (githubMatch) {
      ...
      owner = gitlabMatch[1];
      repoName = gitlabMatch[2];
      fullRepo = `${owner}/${repoName}`;
      isCustomUrl = true;
      customUrl = repoWithoutVersion;
    } else if (codebergMatch) {
      owner = codebergMatch[1];
      repoName = codebergMatch[2];
      fullRepo = `${owner}/${repoName}`;
      isCustomUrl = true;
      customUrl = repoWithoutVersion;
    } else {
      // Arbitrary URL - extract skill name from path
      isCustomUrl = true;
      customUrl = repoWithoutVersion;
      try {
        const url = new URL(repoWithoutVersion);
        const pathParts = url.pathname.split('/').filter(p => p);
        skillName = pathParts[pathParts.length - 1] || url.hostname.replace(/\./g, '-');
        owner = url.hostname;
        repoName = skillName;
        fullRepo = `${owner}/${skillName}`;
      } catch (e) {
    ...
    if (parts.length !== 2) {
      throw new Error('Invalid repo format. Use owner/repo or full URL');
    }
    owner = parts[0];
    repoName = parts[1];
...
/**
 * Fetch .aam.sig signature file from GitHub raw content
 * @param {string} owner - Repo owner
 * @param {string} repo - Repo name
 * @param {string} ref - Git ref (branch/tag) to fetch from
 * @returns {Promise<string|null>} - .aam.sig content or null
 */
async function fetchSigFile(owner, repo, ref) {
  const url = `https://raw.githubusercontent.com/${owner}/${repo}/${ref}/.aam.sig`;
  try {
    const response = await fetch(url);
    if (response.ok) {
...
/**
 * Fetch SKILL.md from a custom URL (GitLab, Codeberg, or arbitrary)
 * @param {string} customUrl - The custom URL
 * @param {string} version - Optional version/tag/branch
 * @returns {Promise<Object|null>} - { content, ref } or null
 */
async function fetchSkillMdFromCustomUrl(customUrl, version) {
  const refs = version ? [version, 'main', 'master'] : ['main', 'master'];

  // Check if it's GitLab
  const gitlabMatch = customUrl.match(/gitlab\.com\/([^\/]+)\/([^\/\.]+)/);
  if (gitlabMatch) {
    const [, owner, repo] = gitlabMatch;
    for (const ref of refs) {
      const url = `https://gitlab.com/${owner}/${repo}/-/raw/${ref}/SKILL.md`;
      try {
        const response = await fetch(url);
        if (response.ok) {
          return { content: await response.text(), ref };
  ...
  // Check if it's Codeberg
  const codebergMatch = customUrl.match(/codeberg\.org\/([^\/]+)\/([^\/\.]+)/);
  if (codebergMatch) {
    const [, owner, repo] = codebergMatch;
    for (const ref of refs) {
      const url = `https://codeberg.org/${owner}/${repo}/raw/branch/${ref}/SKILL.md`;
      try {
        const response = await fetch(url);
        if (response.ok) {
          return { content: await response.text(), ref };
  ...
  // Arbitrary URL - try fetching SKILL.md directly from the URL
  const baseUrl = customUrl.replace(/\/$/, '');
  const skillUrl = `${baseUrl}/SKILL.md`;
  try {
    const response = await fetch(skillUrl);
    if (response.ok) {
      return { content: await response.text(), ref: 'custom' };
    }
  } catch (e) {
    // Failed to fetch
  }
...
/**
 * Fetch .aam.sig from a custom URL (GitLab, Codeberg, or arbitrary)
 * @param {string} customUrl - The custom URL
 * @param {string} ref - Git ref (branch/tag) to fetch from
 * @returns {Promise<string|null>} - .aam.sig content or null
 */
async function fetchSigFileFromCustomUrl(customUrl, ref) {
  // Check if it's GitLab
  const gitlabMatch = customUrl.match(/gitlab\.com\/([^\/]+)\/([^\/\.]+)/);
  if (gitlabMatch) {
    const [, owner, repo] = gitlabMatch;
    const url = `https://gitlab.com/${owner}/${repo}/-/raw/${ref}/.aam.sig`;
    try {
      const response = await fetch(url);
      if (response.ok) {
        return await response.text();
  ...
  if (codebergMatch) {
    const [, owner, repo] = codebergMatch;
    const url = `https://codeberg.org/${owner}/${repo}/raw/branch/${ref}/.aam.sig`;
    try {
      const response = await fetch(url);
      if (response.ok) {
        return await response.text();
  ...
  // Arbitrary URL - try fetching .aam.sig directly
  const baseUrl = customUrl.replace(/\/$/, '');
  const sigUrl = `${baseUrl}/.aam.sig`;
  try {
...
/**
 * Add (install) a skill from a GitHub repository or custom URL
 * @param {string} repo - GitHub repo in format "owner/repo", full URL, or custom URL
...
export async function addFromRepo(repo, options = {}) {
  try {
    ...
    // Parse repo string (now includes version, custom URL, and local path support)
    const { owner, repoName, fullRepo, skillName, version, isCustomUrl, customUrl, isLocalPath, localPath } = parseRepo(repo);
    ...
    if (isLocalPath) {
      const sourceSkillMd = path.join(localPath, 'SKILL.md');
      const sourceSig = path.join(localPath, '.aam.sig');

      if (!fs.existsSync(sourceSkillMd)) {
        return { success: false, error: `SKILL.md not found at ${localPath}` };
      }

      console.log(`Copying skill from ${localPath}...`);

      // Create skill directory
      fs.mkdirSync(targetDir, { recursive: true });

      // Copy SKILL.md
      fs.copyFileSync(sourceSkillMd, path.join(targetDir, 'SKILL.md'));

      // Copy .aam.sig if it exists
      if (fs.existsSync(sourceSig)) {
        fs.copyFileSync(sourceSig, path.join(targetDir, '.aam.sig'));
      }

      // Read SKILL.md content for metadata
      const content = fs.readFileSync(sourceSkillMd, 'utf8');
      let description = '';
    ...
    let skillInfo = {
      name: skillName,
      repo: fullRepo,
      source: isCustomUrl ? customUrl : `https://github.com/${fullRepo}`,
      version: version || 'latest'
    };

    // Try light mode first: just fetch SKILL.md (like skills.sh)
    if (!options.full) {
      const displayName = isCustomUrl ? customUrl : fullRepo;
      console.log(`Fetching SKILL.md from ${displayName}${version ? `@${version}` : ''}...`);

      // Use appropriate fetch function based on URL type
      const result = isCustomUrl
        ? await fetchSkillMdFromCustomUrl(customUrl, version)
        : await fetchSkillMd(owner, repoName, version);
      ...
      if (result) {
        ...
        // Also fetch .aam.sig if it exists (for signature verification)
        const sigContent = isCustomUrl
          ? await fetchSigFileFromCustomUrl(customUrl, result.ref)
          : await fetchSigFile(owner, repoName, result.ref);
        if (sigContent) {
          fs.writeFileSync(path.join(targetDir, '.aam.sig'), sigContent);
        }
    ...
    let repoUrl;
    if (isCustomUrl) {
      // For custom URLs, try to construct a git URL - append .git if not present
      repoUrl = customUrl.endsWith('.git') ? customUrl : `${customUrl}.git`;
    } else {
...
export function listInstalled(options = {}) {
  try {
    ...
    for (const dir of dirs) {
      ...
      const rootSkillMd = path.join(skillPath, 'SKILL.md');

      // Check if this is a multi-skill repo (no root SKILL.md but has nested skills)
      if (!fs.existsSync(rootSkillMd)) {
        const nestedSkills = findNestedSkills(skillPath);
        if (nestedSkills.length > 0) {
          // Add each nested skill as a separate entry
          for (const nestedSkillMd of nestedSkills) {
            const nestedSkillDir = path.dirname(nestedSkillMd);
            const relativePath = path.relative(skillPath, nestedSkillDir);
            const nestedName = `${dir}/${relativePath}`;
            const nestedMetaFile = path.join(nestedSkillDir, '.aam.json');

            let nestedInfo = { name: nestedName, path: nestedSkillDir, parentRepo: dir };

            if (fs.existsSync(nestedMetaFile)) {
              nestedInfo = { ...nestedInfo, ...JSON.parse(fs.readFileSync(nestedMetaFile, 'utf8')) };
            }

            // Check signature status for nested skill
            if (checkSigs) {
              const sigResult = verifySkill(nestedSkillDir, {});
              if (sigResult.success && sigResult.verified) {
                nestedInfo.signatureStatus = 'verified';
                nestedInfo.signedBy = sigResult.pubkey;
              } else if (sigResult.unsigned) {
                nestedInfo.signatureStatus = 'unsigned';
              } else {
                nestedInfo.signatureStatus = 'invalid';
                nestedInfo.signatureError = sigResult.error;
              }
            }

            skills.push(nestedInfo);
          }
          continue; // Skip adding the parent repo as a skill
        }
      }

      // Regular single-skill repo
...
export function signSkill(skillPath, privkey, options = {}) {
  try {
    // Check if skill path exists first
    if (!fs.existsSync(skillPath)) {
...
export function verifySkill(skillPath, options = {}) {
  try {
    // Check if skill path exists first
    if (!fs.existsSync(skillPath)) {
      return { success: false, verified: false, error: `Skill not found: "${skillPath}"` };
    }
    ...
    if (!fs.existsSync(skillMdPath)) {
      // Check if this is a multi-skill repo with nested skills
      if (fs.statSync(skillPath).isDirectory()) {
        const nestedSkills = findNestedSkills(skillPath);
        if (nestedSkills.length > 0) {
          const skillNames = nestedSkills.map(p => {
          ...
          return {
            success: false,
            verified: false,
            error: `This is a multi-skill repo. Verify individual skills:\n  ${skillNames.join('\n  ')}\n\nUsage: aam skills verify <repo-name>/<skill-name>`
          };
        }
      }
      return { success: false, verified: false, error: 'SKILL.md not found in skill directory' };
    }
