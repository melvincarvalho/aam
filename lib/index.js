 ...
 * @param {string} repo - GitHub repo in format "owner/repo", full GitHub URL, or custom URL
...
function parseRepo(repo) {
  ...
  return { owner, repoName, fullRepo, skillName, version, isCustomUrl, customUrl };
}
...
export async function addFromRepo(repo, options = {}) {
  try {
    ...
    // Parse repo string (now includes version and custom URL support)
    const { owner, repoName, fullRepo, skillName, version, isCustomUrl, customUrl } = parseRepo(repo);
    ...
    let repoUrl;
    if (isCustomUrl) {
      // For custom URLs, try to construct a git URL
      if (customUrl.includes('gitlab.com') || customUrl.includes('codeberg.org')) {
        repoUrl = customUrl.endsWith('.git') ? customUrl : `${customUrl}.git`;
      } else {
        // For arbitrary URLs, full clone is not supported
        return { success: false, error: `Full clone not supported for arbitrary URLs. Light mode fetch from ${customUrl} failed.` };
      }
    } else {
