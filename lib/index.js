 ...
 * @param {string} owner - Repo owner
 * @param {string} repo - Repo name
 * @returns {Promise<Object|null>} - Agent files or null
 */
async function fetchAgentMd(owner, repo) {
  const branches = ['main', 'master'];

  for (const branch of branches) {
    // Try root agent.md first
    const url = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/agent.md`;
    try {
      const response = await fetch(url);
      if (response.ok) {
        return { content: await response.text(), path: 'agent.md' };
      }
    } catch (e) {
      // Continue
    }

    // Try agents/ directory pattern
    const agentsUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/agents/${repo}/agent.md`;
    try {
      const response = await fetch(agentsUrl);
      if (response.ok) {
        return { content: await response.text(), path: `agents/${repo}/agent.md` };
      }
    } catch (e) {
      // Continue
    }
...
export async function addAgentFromRepo(repo, options = {}) {
  try {
    ...
    // Parse repo string
    const { owner, repoName, fullRepo, skillName: agentName } = parseRepo(repo);
    ...
    if (!options.full) {
      console.log(`Fetching agent.md from ${fullRepo}...`);
      const agentMd = await fetchAgentMd(owner, repoName);
